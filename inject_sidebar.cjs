...
// The marker we want to insert after:
const targetString = `              <DrawerSection label="એકાઉન્ટ" darkMode={darkMode}>
...
if (!layoutContent.includes('એપ થીમ (Theme)')) {
  layoutContent = layoutContent.replace(targetString, targetString + themeSwitcherSidebarJSX);
}

fs.writeFileSync('src/components/Layout.jsx', layoutContent);
console.log("Safely injected theme switcher into Layout sidebar");
